...
const EnhancedFileManager = () => {
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      toast({
        title: "Success",
        description: `${selectedFiles.length} file(s) moved to recycle bin`,
      });
  ...
  const handleBulkExport = async () => {
    try {
      await exportFiles(selectedFiles);
      toast({
        title: "Success",
        description: `${selectedFiles.length} file(s) exported`,
      });
      setShowBulkActions(false);
    } catch (error) {
      toast({
              ...
              {filteredFiles.map((file, index) => {
                ...
                return (
                  <motion.div
                    ...
                    transition={{ delay: index * 0.1 }}
                  >
                    <Card className={`p-4 cursor-pointer hover:shadow-lg transition-all group relative ${
